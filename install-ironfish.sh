...
#ExecStart=$(which ironfish) miners:start --pool pool.ironfish.network --address $wallet_address
Restart=always
RestartSec=10
LimitNOFILE=10000
[Install]
...
echo -e "\nWallet address: \e[42m$wallet_address\e[0m\n" && sleep 1
echo -e '\n\e[42mUpdate service file\e[0m\n' && sleep 1
echo "[Unit]
Description=IronFish Miner
After=network-online.target
[Service]
User=$USER
#ExecStart=$(which ironfish) miners:start -v -t $IRONFISH_THREADS --no-richOutput
ExecStart=$(which ironfish) miners:start -t $IRONFISH_THREADS --pool pool.ironfish.network --address $wallet_address
Restart=always
RestartSec=10
LimitNOFILE=10000
[Install]
